...
// je stocke un tableau qui va contenir mes erreus.
$formErrors = array();

// On verifie le nombre de lignes dans le tableau POST qui contient toutes les données saisies dans le formulaire.
if (count($_POST) > 0) {
  // On vérifie que la variable $_POST['lastname'] existe ET n'est pas vide
  if (!empty($_POST['lastName'])) {
    // Je vérifie bien que ma variable $_POST['lastname'] correspond à sa pattern
    if (preg_match($patternName, $_POST['lastName'])) {
      // On stocke dans la variable $lastname la variable $_POST['lastname'] dont on a desactivé les balises HTML
...
<html lang="fr" dir="ltr">
  ...
  <body>
    <div class="container">
      <?php
      // On affiche le formulaire si rien n'a été envoyé ou qu'il y a une erreur dans ce qui a été saisi
      if (count($_POST) == 0 || count($formErrors) > 0) {
        ...
        <form method="POST" action="index.php">
          <div class="form-group">
            ...
            <?php
            /* On donne en valeur à notre input ce qui a été saisi par l'utilisateur. ca permet à l'utilisateur de ne pas ressaisir ses données en cas d'erreur.
            ...
            <input type="text" value="<?= isset($_POST['lastName']) ? $_POST['lastName'] : '' ?>" name="lastName" class="form-control" id="lastName" placeholder="Dupont" required />
              <?php
              // On affiche une alerte rouge qui contient le texte de l'erreur s'il y en a une.
         ...
         * On affiche une alerte verte pour prévenir l'utilisateur que s'est bien passé
         * On affiche les variables $lastname, $firstname et $title car elles contiennent la saisie de l'utilisateur si tout s'est bien passé
         * On utilise la balise br/ uniquement dans un <p>
      ...
      <div class="jumbotron">
        ...
        <a href="index.php" class="btn btn-info" title="Retour vers le formulaire">Ajouter un nouvel utilisateur</a>
      </div>
 <?php } ?>
    </div>
  </body>
